...
{
    public class BlueTrollAttackService : MonoBehaviour, TriggerCollider2D.ITriggerCollider2DListener
    {
        ...
        void TriggerCollider2D.ITriggerCollider2DListener.OnTriggerEnter2D(TriggerCollider2D self, Collider2D collider)
        {
            ...
            if (GetComponent<BlueTrollMoodService>().IsAngry)
        ...
        public IEnumerator SmashingRoutine()
        {
            ...
            if (!GetComponent<BlueTrollController>().IsLeaving)
            {
                ...
                if (!GetComponent<BlueTrollController>().IsLeaving)
                {
                    GetComponent<BlueTrollAnimationHelper>().Play(AnimationReferences.BlueTrollStanding);
                }
            }
            isSmashing = false;
        }
